...
void resize_bilinear_c1(const unsigned char* src, int srcw, int srch, int srcstride, unsigned char* dst, int w, int h, int stride)
{
    ...
    int* xofs = buf;     //new int[w];  dst图像宽度往src图像找的位置
    int* yofs = buf + w; //new int[h];  dst图像高度往src图像找的位置

    short* ialpha = (short*)(buf + w + h);    //new short[w * 2];  根据往回找的位置存储的宽度比例系数
    short* ibeta = (short*)(buf + w + h + w); //new short[h * 2];  根据往回找的位置存储的高度比例系数
...
#undef SATURATE_CAST_SHORT
    ...
    // loop body
    ncnn::Mat rowsbuf0(w, (size_t)2u);
    ncnn::Mat rowsbuf1(w, (size_t)2u);
...
#undef SATURATE_CAST_SHORT
    ...
    // loop body
    ncnn::Mat rowsbuf0(w * 2 + 2, (size_t)2u);
    ncnn::Mat rowsbuf1(w * 2 + 2, (size_t)2u);
    short* rows0 = (short*)rowsbuf0.data;
    short* rows1 = (short*)rowsbuf1.data;
                             ...
                             unsigned char* dst, int dstw, int dsth)
{
    ...
    resize_bilinear_c1(srcy, srcw, srch,srcy_stride, dsty, dstw, dsth, dstw);
    unsigned char* dstu=dst+dstw*dsth;
    resize_bilinear_c1(srcu,srcw/2,srch/2,srcu_stride,dstu,dstw/2,dsth/2,dstw/2);
    unsigned char* dstv=dstu+dstw/2*dsth/2;
    resize_bilinear_c1(srcv,srcw/2,srch/2,srcv_stride,dstv,dstw/2,dsth/2,dstw/2);
}

void yuv420p2rgb(const unsigned char* yuv420p, int w, int h, unsigned char* rgb)
{
    const unsigned char* yptr = yuv420p;
    const unsigned char* uptr = yuv420p + w * h;
    const unsigned char* vptr = uptr + w/2 * h/2;
